...
test("generated package files pin discord.js and include a reproducible lockfile", () => {
  ...
  assert.equal(packageJson.scripts["register:commands"], "node dist/register-commands.js");
  assert.equal(files[".env.example"].includes("DISCORD_APPLICATION_ID"), true);
  assert.equal(files[".env.example"].includes("DISCORD_GUILD_ID"), true);
  assert.equal(files["src/register-commands.ts"].includes("Routes.applicationGuildCommands"), true);
  assert.equal(files["src/commands/load.test.ts"].includes("serializable for registration"), true);
});
