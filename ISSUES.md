...
- **Status:** Complete — Android SDK command-line tools and required package directories are now available under `/root/android-sdk`, and the repo-local untracked `local.properties` points Gradle[...]
...
- **Resolution notes:** Resolved on 2026-05-18T09:49:01Z by installing the required SDK under `/root/android-sdk`, writing untracked `local.properties`, and passing `./scripts/android-sdk-prefligh[...]
...
- **Repeat occurrence 2026-05-18T08:33:15Z:** Attempted to resolve the shell `ANDROID_HOME` issue by installing Android command-line tools and writing shell exports.
  - Commands: `./scripts/android-sdk-bootstrap.sh`, `curl -fL https://dl.google.com/android/repository/commandlinetools-linux-13114758_latest.zip`, `apt-get update`, `./scripts/android-sdk-preflig[...]
  - Result: `ANDROID_HOME` and `ANDROID_SDK_ROOT` are now exported for new login shells via `/root/.bash_profile`, and untracked `local.properties` points Gradle at `/root/android-sdk`. Direct dow[...]
  - Status: Incomplete — the unset environment variable is fixed for new shells, but the actual Android SDK packages cannot be installed until this environment has an approved mirror or proxy ac[...]
...
- **Repeat occurrence 2026-05-18T09:49:01Z:** Pre-session unresolved issue review repeated the Android SDK preflight before Android backup hardening work, then retried the bootstrap now that Andro[...]
  - Commands: `./scripts/android-sdk-preflight.sh`, `./scripts/android-sdk-bootstrap.sh`, `sdkmanager --sdk_root=/root/android-sdk "platform-tools" "platforms;android-35" "build-tools;35.0.0"`, `p[...]
...
- **Repeat occurrence 2026-05-18T09:51:47Z:** Android compilation verification for backend URL configuration changes hit the same missing SDK limitation.
  ...
  - Result: `SDK location not found. Define a valid SDK location with an ANDROID_HOME environment variable or by setting the sdk.dir path in your project's local properties file at '/workspace/roy[...]
...
- **Repeat occurrence 2026-05-18T09:58:01Z:** Pre-session unresolved issue review and Android compilation verification for GitHub UI display changes hit the same missing SDK limitation.
  ...
  - Result: Preflight reported `Android SDK preflight failed: ANDROID_HOME is not set and no sdk.dir was found in local.properties.` Gradle reported `SDK location not found. Define a valid SDK loc[...]
  - Status: Incomplete — environment limitation remains; Kotlin/Android compilation must be rerun in an SDK-enabled environment.
- **Repeat occurrence 2026-05-18T10:00:14Z:** Pre-session unresolved issue review and Android unit-test verification for URL path encoding changes hit the same missing SDK limitation.
  ...
  - Results: preflight reported `ANDROID_HOME is not set and no sdk.dir was found in local.properties`; Gradle reported `SDK location not found. Define a valid SDK location with an ANDROID_HOME en[...]
  - Solution attempt: Re-ran the repository preflight as directed and attempted the Android unit-test task after code changes; no SDK root or approved Android SDK artifact mirror is available in t[...]
...
- **Status:** Complete — generated bot installs now pin `discord.js`, include a generated lockfile, document registry mirror configuration, and emit redacted registry diagnostics for install fai[...]
- **Context:** While hardening generated bot installs from `backend/src/services/projectFiles.ts`, this environment returned `403 Forbidden` when attempting to resolve `discord.js@14.15.3` from th[...]
- **Observed details:**
...
- **Troubleshooting steps taken:**
  ...
  - Added a backend-side generated bot lockfile template and verified the generated `package-lock.json` is accepted by `npm ci --package-lock-only --ignore-scripts --audit=false --fund=false` with[...]
...
- **Context:** Durable SQLite persistence introduced synchronous filesystem, OS temp-directory, child-process, and AES-GCM crypto usage in a repository that intentionally uses local `node-shims.d.[...]
