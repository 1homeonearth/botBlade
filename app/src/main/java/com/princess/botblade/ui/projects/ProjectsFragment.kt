    ...
    private fun EmptyProjectState(onAdd: () -> Unit) {
        Card(colors = CardDefaults.cardColors(containerColor = Panel), border = BorderStroke(1.dp, Stroke)) {
            Column(Modifier.fillMaxWidth().padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Text("Start with a Discord TypeScript bot, moderation toolkit, or blank workspace. Import lanes are visible so the page matches the finished workstation path.", color = Muted)
