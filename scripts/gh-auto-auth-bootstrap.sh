...
block=$(cat <<BLOCK
$marker_start
# Run non-interactive gh login when a token secret or stored gh auth is present.
...
# Critical for Codex Cloud:
# secrets are available during setup, then removed before the agent phase.
# Run auth immediately while GH_TOKEN/GITHUB_TOKEN/GH_Token can still be read.
if [ -x "$repo_root/scripts/gh-auto-auth.sh" ]; then
  "$repo_root/scripts/gh-auto-auth.sh" || true
fi